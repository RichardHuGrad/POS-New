...
/*
    ...
    public function addItem() {
        ...
        if (empty($Order_detail)) {
            // to create a new order
            $this->createOrder($restaurant_id, $this->Session->read('Front.id'), $table, $type, $tax_rate);
        } else {
            $order_id = $Order_detail['Order']['id'];
        }
        ...
        $this->updateOrderPrice($order_id, $tax_rate, $Order_detail, $item_detail);
    ...
    public function updateOrderPrice($order_id, $tax_rate, $Order_detail, $item_detail) {
        $this->layout = false;
        $this->autoRender = NULL;
    ...
    public function createOrderItem($order_id, $item_id, $name_en, $name_xh, $price, $category_id, $all_extras, $tax, $tax_amount, $qty) {
     ...
     public function addItem1() {
        ...
        // get tax by restaurant id
        // cashier.restaurant id link to admin.id
        $this->loadModel('Cashier');
        $tax_detail = $this->Cashier->find("first", array(
            'fields' => array('Admin.tax', 'Admin.id'),
            'conditions' => array('Cashier.id' => $this->Session->read('Front.id'))
        ...
        //Modified by Yishou Liao @ Dec 15 2016
        // show combo option, if the cousine item is double combo or triple combo
        $get_comb_flag = $this->Cousine->query("SELECT extras_num FROM extrascategories WHERE id = " . $item_detail['Cousine']['comb_num']);
        $show_extras_flag = false;
        if (count($get_comb_flag)>0){
            if ($get_comb_flag[0]['extrascategories']['extras_num']>0){
