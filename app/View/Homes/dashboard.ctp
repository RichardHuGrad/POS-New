                    ...
                    for ($i = 1; $i <= $tables['Admin']['no_of_tables']; $i++) {
                        ...
                        <li class="clearfix dine_li" style="<?php echo @$dine_table_order[$i - 1] ?>">
                            <ul class="dropdown-menu dine-tables">
                                <div class="arrow"></div>
                                <li <?php if (@$dinein_tables_status[$i] == 'P') echo 'class="disabled"'; ?>><a tabindex="-1" href="<?php
                                    if (@$dinein_tables_status[$i] <> 'P')
                                        echo $this->Html->url(array('controller' => 'homes', 'action' => 'order', 'table' => $i, 'type' => 'D'));
                                    else
                                ...
                                <li <?php if (@$dinein_tables_status[$i] <> 'N' and @ $dinein_tables_status[$i] <> 'V') echo 'class="disabled"'; ?>><a tabindex="-1" href="<?php
                                    if (@$dinein_tables_status[$i] == 'N' OR @ $dinein_tables_status[$i] == 'V')
                                        echo $this->Html->url(array('controller' => 'homes', 'action' => 'pay', 'table' => $i, 'type' => 'D'));
                                    else
                                        echo "javascript:void(0)";
                                    ?>">Pay<br/>结账</a></li>

                                <li <?php if (@$dinein_tables_status[$i] == 'A') echo 'class="disabled"'; ?>><a tabindex="-1" href="<?php
                                    if (@$dinein_tables_status[$i] <> 'A')
                                        echo $this->Html->url(array('controller' => 'homes', 'action' => 'makeavailable', 'table' => $i, 'type' => 'D', 'order' => @$orders_no[$i]['D']));
                                    else
                                        echo "javascript:void(0)";
                                    ?>">Make Available<br/>变空桌</a></li>
                            ...
                            <div class="<?php
                            if (isset($dinein_tables_status[$i]))
                                echo $colors[$dinein_tables_status[$i]];
                            else
                                echo 'availablebwrap';
                            ?> clearfix  dropdown-toggle" data-toggle="dropdown">
                                <div class="number-txt for-dine">堂食<?php echo str_pad($i, 2, 0, STR_PAD_LEFT); ?></div>

                                <div class="order_no_box <?php
                                if (isset($dinein_tables_status[$i]))
                                    echo "whitecolor";
                                else
                                    echo "lightcolor";
                                ...
                                <div class="txt12 text-center <?php
                                if (isset($dinein_tables_status[$i]))
                                    echo "whitecolor";
                                else
                                    echo "lightcolor";
                                ?>"><?php if (@$dinein_tables_status[$i]) { ?> <?php echo @$orders_time[$i]['D'] ? date("H:i", strtotime(@$orders_time[$i]['D'])) : "" ?><?php } ?>
                    ...
                    for ($i = 1; $i <= $tables['Admin']['no_of_takeout_tables']; $i++) {
                        ...
                        <li class="clearfix">
                            <ul class="dropdown-menu">
                                ...
                                <li <?php if (@$takeway_tables_status[$i] <> 'N' and @ $takeway_tables_status[$i] <> 'V') echo 'class="disabled"'; ?>><a tabindex="-1" href="<?php
                                    if (@$takeway_tables_status[$i] == 'N' OR @ $takeway_tables_status[$i] == 'V')
                                        echo $this->Html->url(array('controller' => 'homes', 'action' => 'pay', 'table' => $i, 'type' => 'T'));
                                    else
                                        echo "javascript:void(0)";
                                    ?>">Pay<br/>结账</a></li>
                                <li <?php if (@$takeway_tables_status[$i] <> 'P') echo 'class="disabled"'; ?>><a tabindex="-1" href="<?php
                                    if (@$takeway_tables_status[$i] == 'P')
                                        echo $this->Html->url(array('controller' => 'homes', 'action' => 'makeavailable', 'table' => $i, 'type' => 'T', 'order' => @$orders_no[$i]['T']));
                                    else
                                        echo "javascript:void(0)";
                                    ?>">Make Available<br/>变空桌</a></li>
                            </ul>
                            <div class="<?php
                            if (isset($takeway_tables_status[$i]))
                                echo $colors[$takeway_tables_status[$i]];
                            else
                                echo 'availablebwrap';
                            ?> clearfix  dropdown-toggle" data-toggle="dropdown">
                                ...
                                if (@$takeway_tables_status[$i]) {
                                    ...
                                    if (isset($takeway_tables_status[$i]))
                                        ...
                                        echo "lightcolor";
                                    ?>"><?php echo @$orders_time[$i]['T'] ? date("H:i", strtotime(@$orders_time[$i]['T'])) : "" ?></div>
                    ...
                    for ($i = 1; $i <= $tables['Admin']['no_of_waiting_tables']; $i++) {
                        ...
                        <li class="clearfix">
                            <ul class="dropdown-menu">
                                ...
                                <li <?php if (@$waiting_tables_status[$i] <> 'N' and @ $waiting_tables_status[$i] <> 'V') echo 'class="disabled"'; ?>><a tabindex="-1" href="<?php
                                    if (@$waiting_tables_status[$i] == 'N' OR @ $waiting_tables_status[$i] == 'V')
                                        echo $this->Html->url(array('controller' => 'homes', 'action' => 'pay', 'table' => $i, 'type' => 'W'));
                                    else
                                        echo "javascript:void(0)";
                                    ?>">Pay<br/>结账</a></li>
                                <li <?php if (@$waiting_tables_status[$i] <> 'P') echo 'class="disabled"'; ?>><a tabindex="-1" href="<?php
                                    if (@$waiting_tables_status[$i] == 'P')
                                        echo $this->Html->url(array('controller' => 'homes', 'action' => 'makeavailable', 'table' => $i, 'type' => 'W', 'order' => @$orders_no[$i]['W']));
                                    else
                                        echo "javascript:void(0)";
                                    ?>">Make Available<br/>变空桌</a></li>
                            </ul>
                            <div class="<?php
                            if (isset($waiting_tables_status[$i]))
                                echo $colors[$waiting_tables_status[$i]];
                            else
                                echo 'availablebwrap';
                            ?> clearfix  dropdown-toggle" data-toggle="dropdown">
                                ...
                                <div class="txt12 text-center <?php
                                     if (isset($waiting_tables_status[$i]))
                                         ...
                                         echo "lightcolor";
                                     ?>">
                                         <?php if (@$waiting_tables_status[$i]) { ?> <?php
                                             echo @$orders_time[$i]['W'] ? date("H:i", strtotime(@$orders_time[$i]['W'])) : "";
                                         }
                                         ?>
                                         <?php echo $this->Html->image('seat.png', array('alt' => 'Seat', 'title' => 'Seats')); ?> 
                                    <?php echo @$dine_table[$i - 1] ?></div>
